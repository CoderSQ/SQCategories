...
@implementation UIDevice (SQHardware)
...
+ (NSUInteger)sq_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)sq_cpuFrequency {
    return [self sq_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)sq_busFrequency {
    return [self sq_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)sq_ramSize {
    return [self sq_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)sq_cpuNumber {
    return [self sq_getSysInfo:HW_NCPU];
}


+ (NSUInteger)sq_totalMemoryBytes
{
    return [self sq_getSysInfo:HW_PHYSMEM];
}
